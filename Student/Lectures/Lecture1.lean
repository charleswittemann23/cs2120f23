...
| true, false => false 
| false, true => false 
| false, false => true  -- delete this line and fill in the four cases
...
| troo => troo
def id_bool2 : Bool -> Bool
| true => true
| false => false
def id_bool3 (d: Bool) : Bool := d

def id_string  (s: String): String := s
def id_polymorphic_universal (T : Type)  :  T → T
| v => v
def id_poly' {v : Type} : v → v
| v=>v
#eval id_poly' "Hello"