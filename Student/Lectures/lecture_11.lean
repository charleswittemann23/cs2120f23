...
### Atomic Propositions
-- You can ask if a proposition is true. Is it true that ______?
...
-- a function that takes in 2 variables and each returns a  bool for each interpretation
-- you can think of interpretations as different worlds or scenarios.
...
-/
--just like inductive, but when you only have one constructor. 
structure var : Type := 
(n: Nat)
...
#eval v₂.n      -- dot notation, takes value of n-field
...
| false , false => true 
| _ , _ => false 
--max means that not operator takes precedence over other operators 
--operator in front of the argument
...
-- infixr means declaring it between arguments
