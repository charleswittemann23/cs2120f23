...
| fun (Sum.inl a , Sum.inl y) => Sum.inl (Sum.inl (a, y))
| fun (Sum.inl a , Sum.inr d) => Sum.inl (Sum.inr (a, d))
| fun (Sum.inr b, Sum.inl y) => Sum.inr (Sum.inl (b,y ))
| fun (Sum.inr b, Sum.inr d) => Sum.inr (Sum.inr (b, d))

-- Write the second function here from scratch
def or_prods_to_prod_ors {α β γ δ : Type} : 
(α × γ ⊕ α × δ ⊕ β × γ ⊕ β × δ) → (α ⊕ β ) × (γ ⊕ δ)
|fun Sum.inl (Sum.inl (a, y)) => (Sum.inl a , Sum.inl y)
|fun Sum.inl (Sum.inr (a, d)) => (Sum.inl a , Sum.inr d)
|fun Sum.inr (Sum.inl (b, y)) => (Sum.inr b , Sum.inl y)
|fun Sum.inr (Sum.inr (b, d)) => (Sum.inr b, Sum.inr d)
...
def my_prop := (A ∨ O) ∧ (C ∨ B) ⇔ (A ∧ C) ∨ (A ∧ B) ∨ (O ∧ C) ∨ (O ∧ B)
