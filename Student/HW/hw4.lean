...
-- Here:
def prod_com_reverse{ α β : Type} : α × β → β × α 
| a => Prod.mk a.2 a.1
/-! 
...
| a => Prod.mk (Prod.mk a.1 a.2.1) a.2.2
...
-- Here:
def prod_assoc_reverse {α β : Type}: (α × β) × γ → α × (β × γ )
| a => Prod.mk a.1.1 (Prod.mk a.1.2 a.2)  
/-!
...
a *γ* value. 
 ...
 | (a, Sum.inl b) => Sum.inl (Prod.mk a b)
 | (a, Sum.inr c) => Sum.inr (Prod.mk a c)
...
| Sum.inr (Prod.mk a c) => Prod.mk a (Sum.inr c)
...
-- Here
def its_wet {a: rain→ wet} {b: sprinkler → wet}: (rain ⊕ sprinkler) →a → b → wet
| Sum.inl c => (a (c))
| Sum.inr d => (b:sprinkler → wet) d
/-!
...
-- Here:
def sum_elim {α β γ : Type} :(α ⊕ β ) → (β → γ) → (α → γ ) → γ 
|Sum.inl a, _, c => (c (a))
|Sum.inr b, x, _ => (x (b))
/-!
